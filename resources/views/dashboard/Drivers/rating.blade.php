...
@section('create_btn'){{ URL('dashboard/driver/add_edit') }}@endsection
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '.ajax_delete_all', function() {
                $.ajax({
                    url: "{{ route('dashboard_city.deleted_all') }}",
                    method: "get",
                    data: {
                        "array": array,
                    },
            ...
            $('.btn_deleted').on("click", function() {
                var id = $('#iddel').val();
                $.ajax({
                    url: "{{ URL('dashboard/driver/deleted') }}",
                    method: "get",
                    data: {
                        "id": id,
                    },
            ...
            $(document).on('change', '.input-priority', function(e) {
                ...
                $.ajax({
                    url: "{{ route('dashboard_city.priority') }}",
                    method: "get",
                    data: {
                        id,
                        priority,
        ...
        var Render_Data = function(type) {
            datatabe = $('#data_Table').DataTable({
                ...
                "ajax": {
                    "url": "{{ URL('dashboard/driver/get_rating_data') }}",
                    "dataType": "json",
                    "type": "POST",
                    "data": {
                        _token: "{{ csrf_token() }}",
        ...
        $(document).on('click', '.btn_confirm_email_current', function() {
                ...
                $.ajax({
                    url: "{{ URL('dashboard/driver/confirm_email') }}",
                    method: "get",
                    data: {
                        "id": id,
                    },
